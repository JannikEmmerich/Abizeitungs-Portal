...
<div id="main">
	...
	<br>
	<h3>Zeige alle Umfragen mit....</h3>  
	<form method="POST">
		<h4>Schüler</h4>
		<input name="value" list="studentslist" autocomplete="off">
	...
	<?php
		if(array_key_exists("value", $_POST) && array_key_exists("type", $_POST)){
			$sel = $_POST["value"];
			$ps = $mysqli->prepare("SELECT COUNT(survey_key) as number, survey_key FROM surveys WHERE survey_value = ? GROUP BY survey_key, survey_value");
			$ps->bind_param("s", $sel);
			$ps->execute();
			$result = $ps->get_result();
			echo '<br>' . "\n";
			...
			if(mysqli_num_rows($result) != 0){
				while($row = $result->fetch_assoc()){
					echo '<b>' . $row["survey_key"] . '</b> mit ' . $row["number"] . " Stimme(n) <br> \n";
				}
			}else{
				echo 'in keiner Umfrage enthalten<br>' . "\n";
			}
		}
	?>
</div>
